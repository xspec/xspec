...
This document proposes a design for extending XSpec capabilities for testing XProc steps (PR #2227).
...
1. Pick up defaults of inputs and options when the XSpec test case does not specify values.
...
Sample Windows syntaxes from the root of the XSpec repository, assuming `XMLCALABASH3_JAR` is set to the path to `xmlcalabash-app-3.0.42.jar` (or latest available version):
...
An XProc step that I call the "step runner" connects the XSLT test runner with the XProc world. The step runner, whose source is `src/common/step-runner.xpl`:
