      ...
      <xsl:variable as="xs:string" name="regex-for-UQName">
         <xsl:value-of xml:space="preserve">
            <!-- based on https://github.com/xspec/xspec/blob/fb7f63d8190a5ccfea5c6a21b2ee142164a7c92c/src/schemas/xspec.rnc#L329 
                 but without ^ and $ anchors at beginning and end, respectively -->
               Q\{
                  ([^\{\}]*)                                     <!-- URI -->
               \}
      ...
      <xsl:value-of>
         <xsl:analyze-string flags="x" regex="{$regex-for-UQName}" select="$as-attribute-value">
            <xsl:matching-substring>
               <!-- First, preserve UQNames already present to avoid confusion if a URI contains a colon -->
               <xsl:sequence select="." />
            </xsl:matching-substring>
            <xsl:non-matching-substring>
               <!-- In non-matches, convert lexical QNames to UQNames -->
               <xsl:analyze-string regex="\s*({$equtil:capture-NCName}:{$equtil:capture-NCName})\s*" select=".">
                  <xsl:matching-substring>
                     <xsl:variable name="lexical-qname" select="regex-group(1)" as="xs:string"/>
                     <xsl:sequence select="x:UQName-from-EQName-ignoring-default-ns($lexical-qname, $context)"/>                  
                  </xsl:matching-substring>
                  <xsl:non-matching-substring>
