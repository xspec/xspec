...
<xsl:stylesheet version="3.0"
    xmlns:local="urn:x-xspec:compiler:xproc:compile:get-step-inputs:local"
    ...
    <!-- Functions -->
    <xsl:function name="x:get-step-inputs" as="element(p:input)*">
        ...
        <xsl:choose>
            <xsl:when test="exists($p-declare-step)">
                <xsl:sequence select="$p-declare-step/p:input"/>
            </xsl:when>
            <xsl:otherwise>
