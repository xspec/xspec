...
<xsl:stylesheet version="3.0"
    ...
    <xsl:template match="@location" as="text()" mode="make-predicate">
        <xsl:text expand-text="yes">[{x:known-UQName('x:schematron-location-compare')}({x:quote-with-apos(.)}, @location, preceding-sibling::svrl:ns-prefix-in-attribute-values)]</xsl:text>
    </xsl:template>
