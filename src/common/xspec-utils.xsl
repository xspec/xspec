...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0"
	...
	-->
	<xsl:function as="xs:string" name="x:known-UQN">
		...
		<xsl:sequence select="x:URIQualifiedName($namespace, $local-name)" />
	</xsl:function>

	<!--
		Returns URIQualifiedName of the variable generated by mode="test:generate-variable-declarations"
	-->
	<xsl:function as="xs:string" name="x:variable-name">
		...
		<xsl:for-each select="$source-element">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:sequence
						select="x:known-UQN('impl:' || local-name() || '-' || generate-id())" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:for-each>
	</xsl:function>
