...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xquery31">
    <!-- This file is a copy of test/schematron/schematron-text.sch, except:
        - queryBinding="xquery31"
        - context="//section" on sch:rule
        - No whitespace-only text nodes within sch:property[@id='prop-data']
    -->
    <sch:pattern>
    ...
    <sch:properties>
        ...
        <!-- XQS preserves WSOT nodes in sch:property, so avoid having them -->
        <sch:property
            id="prop-data"><user-data><elem1
                    id="id1">user content</elem1><elem2
                        id="id2">More user content</elem2></user-data></sch:property>
    </sch:properties>
</sch:schema>