...
@test "running XSpec via ant for Schematron support" {
    ...
    echo ${PWD}
    run ant -buildfile ../build.xml -Dxspec.xml=tutorial/schematron/demo-03.xspec -lib /tmp/xspec/saxon/saxon9he.jar -lib /tmp/ant/lib/xmltask.jar -lib /tmp/ant/lib/ant-contrib.jar -Dtest.type=s -Dxspec.project.dir=${PWD}/../ -Dxspec.compiled.xsl.dir=${PWD}/../tutorial/schematron -Dxspec.phase=#ALL
	echo $output
    [ "$status" -eq 0 ]
    [[ "${output}" =~  "BUILD SUCCESSFUL" ]]
}
