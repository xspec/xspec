...
@test "x:call[@function] with x:context" {
    ...
    echo "$output"
    [ "$status" -eq 1 ]
    [ "${lines[3]}" = "ERROR in x:scenario ('x:call[@function] with x:context'): Setting a context and calling a function at the same time is supported only when /x:description has @run-as='external'." ]
    [ "${lines[${#lines[@]}-1]}" = "*** Error compiling the test suite" ]
}
