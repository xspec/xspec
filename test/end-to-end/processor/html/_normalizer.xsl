...
<xsl:stylesheet exclude-result-prefixes="#all" version="3.0"
	...
	<!--
		Normalizes SVRL in Schematron Result
			...
			Example (SchXslt):
				...
				out: <svrl:active-pattern documents="demo-02.xml"
				...
				out: <skos:prefLabel>SchXslt/1.6.2 SAXON/EE 9.9.1.7</skos:prefLabel>
	-->
	...
	<xsl:function as="xs:string?" name="local:svrl-creator">
		...
		<xsl:if test="$svrl-pre">
			<!-- parse-xml() fails when the string is XML 1.1. Inspect the literal string instead. -->
			<xsl:variable as="xs:string" name="schxslt-xmlns"
				>xmlns:schxslt="https://doi.org/10.5281/zenodo.1495494"</xsl:variable>
			<xsl:choose>
				<xsl:when
					test="$svrl-pre/span[contains-token(@class, 'xmlns')][. eq $schxslt-xmlns]">
